...
#     https://colab.research.google.com/drive/1y3G-OA3tvgjRuIAhLWqwleKHqBuQtCEY
# """
...
import glob
import os
# >>>>>>> 25fca43d1cacc2354b81b213c440fd1431ca342e

def load_and_concat_data(directory_path):
#     """
#     Reads all CSV files in a directory, concatenates them, and adds a 'Scenario' column.

#     Args:
#         directory_path (str): The path to the directory containing the CSV files.

#     Returns:
#         pd.DataFrame: A concatenated DataFrame with a 'Scenario' column.
#     """
...
# # Load and concatenate data from the current directory
...
all_data_melted = all_data_df.melt(
    id_vars=['tableName', 'seriesName', 'label', 'Scenario'],
    value_vars=years,
    var_name='Year',
    value_name='Value'
)
...
# display(all_data_melted.head())

# all_data_melted = pd.read_csv('all_data_melted.json')

# all_data_melted.to_json('all_data_melted.json', orient='records', indent=2)
...
app.layout = html.Div([
    ...
    dcc.RangeSlider(
        ...
        max= all_data_melted['Year'].max(),
        value=[2018, 2050],
        marks={str(year): str(year) for year in range(all_data_melted['Year'].min(), all_data_melted['Year'].max()+1, 1)},
        step=1
    ),
    dcc.Tabs([
